...
def coerce_value(value, kind, where):
    if value is None:
        return None
    if kind == "string":
        return str(value)
            ...
            for field, kind in fields:
                if field in row and row[field] is not None:
                    row[field] = coerce_value(
                        row[field], kind, "{} row {}.{}".format(name, index, field))
...
def publish_report(report, payload, log=None):
    ...
    try:
        ...
        for name, fields in TABLES:
            ...
            report.CreateTable(native_name)
            for field, kind in fields:
                context = "CreateField({}.{})".format(native_name, field)
                report.CreateField(native_name, field, FIELD_TYPES[kind])
            for index, row in enumerate(payload[name]):
                for field, _ in fields:
                    ...
                    report.SetValue(native_name, field, index, value)
            if log:
